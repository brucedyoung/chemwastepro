...
def itemedit():
    ...
    topactionbutton2 = A(DIV(' Shelve Item', _class='fa fa-sign-in'), _class='btn btn-block btn-default btn-xs', component=URL('shelve'+'?item_id='+str(item_id)+'&item_chemname='+chemname), target='target1')
...
def chemindex():
    ...
    chemindex = json.dumps([dict(id=r.id,chemname=r.chemname,group=r.group_,state=r.state,disposalcode=r.disposalcode,tsdf=r.tsdf,treatment=r.treatment,hazwastecodes=r.hazwastecodes,hazard=r.hazard,editlink=icon  % r.id) for r in rows.render()])#adding render to use represented view of fields
...
def chemindexedit():
    ...
    fields=['chemname', 'casnum', 'group_', 'state', 'disposalcode', 'packtype', 'treatment','components','hazard', 'hazwastecodes', 'comments']#
    if (request.args(0)): #UPDATE RECORD or INSERT RECORD
...
def shelve():
    ...
    return (request.vars.item_id+request.vars.item_chemname)
